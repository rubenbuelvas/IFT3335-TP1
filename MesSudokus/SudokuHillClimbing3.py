...
def displaygridv(gv_init, gv_current, showconflicts=True):
    ...
    if showconflicts: # Will evaluate conflicts in order to show them
        gv_conflicts, nb_conflicts, conflicts_grid_values = evalconflicts(gv_init, gv_current)  # Will initiale a grid_value containing number of conflicts

    # Header of the grid
    if showconflicts: displaystring = '-------- VALUE GRID ---------    --- CONFLICTS GRID (' + str(nb_conflicts).ljust(3) + ') ----\n' # header
...
def improve_solution_hill_climb_calc_all_swaps3x3(gv_init, gv_current):
    ...
    for r in rows:
        for c in cols:
            if r+c in all_swappables_squares:
                ...
                possible_swaps = set(squares_within_unit_list(r+c,"unit3x3")) - {r+c}  # squares in unit, except current
                ...
                for s in possible_swaps:
                    ...
                    if r+c < s: set_of_swappable_pairs.add((r+c, s))
                    else: set_of_swappable_pairs.add((s, r+c))

    # Will simulate each swap, calculate the total conflicts for each swap and choose the best
    best_puzzle = gv_current
    best_total_conflicts = countconflicts(gv_init, gv_current)
    ...
    if best_puzzle == gv_current: # no improvement (local maximum or solution)
        if print_display != 'nothing': print(f"FINAL SOLUTION: found maximum with {best_total_conflicts} conflicts.")
        return gv_current
        ...
        return improve_solution_hill_climb_calc_all_swaps3x3(gv_init, best_puzzle)
