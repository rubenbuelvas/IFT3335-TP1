        ...
        for square_in_same_unit in unit:  # We could optimize here and avoid looking at squares with only one possible value
            ...
            if (len(values[s]) == 2) and (square_in_same_unit != s) and values[square_in_same_unit] == values[s]:  # a naked pair is found
                ...
                for s2 in listofsquarestoreview:
                    ...
                    possibledigits = re.sub(values[s], '', values[
                        s2])  # DG removes the values of the naked pair from the values of s2 (square to review)
                    ...
                    if (numberofvaluesfinal < numberofvaluesinitial) and (
                            numberofvaluesfinal == 1):  # DG we solved a square because of the naked pair, which makes it a better candidate
                        countnakedimprovements += 1  # Number of naked pairs or triples found
                        if printwhenfound:
                            print(
                                f"Solved square {s2} with possible digits {possibledigits} because of naked pair ''{values[s]}'' found for squares {s} and {square_in_same_unit}")
                            values[
                                s2] = possibledigits  # DG update the possibilities for s2. In this case, only one possibilily
...
def findbettersquarewithpairsandtriples(values_in, printwhenfound=True):  # DGNEW    <--------------- THIS CODE CAN BE OPTIMIZED
    ...
    for s in values_pairs:
        ...
        for unit in units[s]:  # Will loop through the 3 units tied to the square
            for square_in_same_unit in unit:  # We could optimize here and avoid looking at squares with only one possible value
                # Looks for pairs
                if (len(values_in[s]) == 2) and (square_in_same_unit != s) and values_in[square_in_same_unit] == \
                        values_in[s]:  # a naked pair is found
                    # if printwhenfound:
                    #    print(f"Naked pair ''{values[s]}'' found for squares {s} and {square_in_same_unit}")
                    ...
                    for s2 in listofsquarestoreview:
                        ...
                        possibledigits = re.sub(values_in[s], '', values_in[
                            s2])  # DG removes the values of the naked pair from the values of s2 (square to review)
                        numberofvaluesfinal = len(possibledigits)
                        # print(f"Found square {s2} with value {newvalue} because of naked pair ''{values[s]}'' found for squares {s} and {square_in_same_unit}")
                        if (numberofvaluesfinal < numberofvaluesinitial) and (
                            ...
                            if printwhenfound:
                                print(
                                    f"Solved square {s2} with possible digits {possibledigits} because of naked pair ''{values_in[s]}'' found for squares {s} and {square_in_same_unit}")
                                values_in[
                                    s2] = possibledigits  # DG update the possibilities for s2. In this case, only one possibilily
                            return s2, possibledigits
                    # break # NOT SURE WE NEED A BREAK HERE. We found a naked pair, but it doesn't mean it will solve another square
